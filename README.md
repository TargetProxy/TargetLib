...
*   🧭 **Smart Connect 控制面**：目标架构由核心持有服务策略、节点偏好、探测调度、候选评分、切换授权、绑定事务和审计；客户端只提交意图并显示状态。
*   ⚙️ **配置引擎**：根据应用设置与解析出的订阅节点，动态生成安全、受控的 `sing-box` 配置文件。
*   🚀 **生命周期与监控**：全面管理 `sing-box` 的启动、停止与配置热加载（Hot-Reload），并实时接管运行状态、日志输出和流量统计。
*   📱 **后台独立运行**：Android 由前台 VPN service 持有 native runtime；目标状态下即使 Flutter UI 被挂起，核心调度和已授权切换仍可继续。
...
Smart Connect 的核心所有权、后台编排、切换状态机、Android 生命周期和迁移计划见：
👉 **[docs/SMART_CONNECT_ARCHITECTURE.md](docs/SMART_CONNECT_ARCHITECTURE.md)**
